...
dat.cov <- read.csv(paste0(filePathData, "mcv1_delayed_covariates_selected.csv"), header=TRUE)
...
dat1 <- group_by(dat, DHSCLUST) %>%
  summarize(total = n(), 
            ...
            mcv1_early_count = sum(mcv1_early, na.rm = TRUE),
            mcv1_early_prop = mcv1_early_count/total_mcv1)
...
vaxcov  <- data.merge[,10:13]   #includes coordinates as Longitude & Latitude
vaxdata <- data.merge[,c(2:9,14:15)]

#################################################################################################
##Create dataset to plot clusterplot of mcv1_delayed
clustplot <- vaxdata %>% 
  mutate(mcv1_delayed_prop = mcv1_delayed_prop*100) %>% 
  select(mcv1_delayed_prop, longitude, latitude) %>% 
  rename(outcome = mcv1_delayed_prop)

##Create dataset for clusterplot of mcv1_timely
clustplot_early <- vaxdata %>% 
  mutate(mcv1_early_prop = mcv1_early_prop*100) %>% 
  select(mcv1_early_prop, longitude, latitude) %>% 
  rename(outcome = mcv1_early_prop)
...
##plot spatial distribution of early mcv1 in The Gambia
...
ggplot(gmb_shp) + 
  ...
  labs(title="Cluster-level observed early MCV1")


##plot spatial distribution of delayed mcv1 in The Gambia
ggplot(gmb_shp) + 
  geom_sf() + 
  ...
  theme_bw()+
  theme(legend.key.height = unit(1.1, 'cm'), panel.grid.minor = element_blank(), panel.grid.major = element_blank(), axis.title = element_blank())+
  labs(title="Cluster-level observed delayed MCV1")
...
ggplot() +
  ...
  labs(title="Cluster-level observed delayed MCV1", fill = "ADM1_EN") +
...
#3. save the image as PNG using name (clustplot_delayed_mcv1) with aspect ratio with = 1000 and height = 400
######################################################################################################################################
...
#All_ages
Numvacc    <- vaxdata$mcv1_delayed_count
...
xp2	    <- vaxcov$GMB_n99_avg_modis_Night_time_landsurface_temp_2016.2019_mean
xp3	    <- vaxcov$GMB_n102_avg_potential_evapotranspiration_2016_2019_mean
xp4     <- vaxcov$urban_rural
...
x2gp  	<- getValues(GMB_n99)			
x3gp	<- getValues(GMB_n102)
x4gp	<- getValues(GMB_urban_rural) 
...
pred.dat <- cbind(Pred_grid2, x1gp, x2gp, x3gp, x4gp, popc) # 
...
pop <- pred.dat.1[,7]              #Population counts for weighted aggregation
...
# Observation points
X0 <- model.matrix(~ -1 + xp1 + xp2 + xp3 + factor(xp4))
Xobs <-  as.data.frame(X0[,-which(colnames(X0)%in%c("factor(xp4)0"))])
colnames(Xobs) <- c("x1", "x2", "x3", "x4_1") # 
...
X0 <- model.matrix(~ -1 + xpred1 + xpred2 + xpred3 + factor(xpred4))
#
Xpred <-  as.data.frame(X0[,-which(colnames(X0)%in%c("factor(xpred4)0"))])
colnames(Xpred) <- c("x1", "x2", "x3", "x4_1") #
...
formula  <- y ~ -1 + intercept + x1 + x2 + x3 + x4_1 + f(s, model=spde) + f(rr, model="iid", hyper = hyper.prec) #
...
write.csv(param.all, paste0("parameter_output_mcv1_delayed.csv"))
...
write.csv(ds, paste0("obsvpred_mcv1_delayed.csv"))
...
idX <- contents$start[which(contents$tag=="intercept")]-1 + (1:5) # fixed effects, 5 = no of regression coefficients
...
save(inv.linpred, file=paste0("simprob_mcv1_delayed.rda"))


####IMMEIATELY RUN THE "MCV1_timely_early script to generate admin level estimates and rasters for delayed.
