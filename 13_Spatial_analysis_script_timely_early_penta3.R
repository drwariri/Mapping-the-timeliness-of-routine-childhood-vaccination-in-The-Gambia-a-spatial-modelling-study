...
dat.cov <- read.csv(paste0(filePathData, "penta3_timely_covariates_selected.csv"), header=TRUE)
...
clustplot <- vaxdata %>% 
  mutate(penta3_timely_prop = penta3_timely_prop*100) %>% 
  select(penta3_timely_prop, longitude, latitude) %>% 
  rename(outcome = penta3_timely_prop)

#read in gambia shapefile
gmb_shp <- st_read(paste0(filePathData2,"gmb_admbnda_adm2_2022.shp"))

##plot spatial distribution of timely penta3 in The Gambia
#scale_colour_gradientn(colours = terrain.colors(10), name="% timely")# another way of changing colours
...
#3. save the image as PNG using name (clustplot_timely_penta3) with aspect ratio with = 1000 and height = 400
######################################################################################################################################
...
#All_ages
Numvacc    <- vaxdata$penta3_timely_count
...
#Covariates
xp1     <- vaxcov$GMB_n11_distance_settlement_guf_ghsl_ESACCI_2014_
xp2     <- vaxcov$GMB_n93_avg_precip_2016_2019
xp3     <- vaxcov$GMB_n98_avg_modis_EVI_2016.2019_mean
xp4	    <- vaxcov$GMB_n99_avg_modis_Night_time_landsurface_temp_2016.2019_mean
xp5     <- vaxcov$l_GMB_n101_avg_max_temperature_2016_2019_mean
xp6     <- vaxcov$GMB_n102_avg_potential_evapotranspiration_2016_2019_mean
xp7     <- vaxcov$urban_rural


#Read in prediction covariates
GMB_n11  	<- raster(paste0(filePathData1,"GMB_n11_distance_settlement_guf_ghsl_ESACCI_2014.tif"))
GMB_n93  	<- raster(paste0(filePathData1,"GMB_n93_avg_precip_2016_2019.tif"))
GMB_n98  	<- raster(paste0(filePathData1,"GMB_n98_avg_modis_EVI_2016-2019_mean.tif"))
GMB_n99  	<- raster("../000_covariates for modelling/GMB_n99_avg_modis_Night_time_landsurface_temp_2016-2019_mean.tif") 
l_GMB_n101 	<- raster(paste0(filePathData1,"GMB_n101_avg_max_temperature_2016_2019_mean.tif"))
...
x1gp 	<- getValues(GMB_n11) 
x2gp 	<- getValues(GMB_n93) 
x3gp 	<- getValues(GMB_n98)
x4gp 	<- getValues(GMB_n99)
x5gp 	<- log(getValues(l_GMB_n101) + 0.05) 
x6gp 	<- getValues(GMB_n102)
x7gp	<- getValues(GMB_urban_rural) 


#Prediction grid
n25.p      <- raster(paste0(filePathData1,"GMB_n11_distance_settlement_guf_ghsl_ESACCI_2014.tif"))
...
write.csv(param.all, paste0("parameter_output_penta3_timely.csv"))
...
write.csv(ds, paste0("obsvpred_penta3_timely.csv"))
...
save(inv.linpred, file=paste0("simprob_output_penta3_timely.rda"))


###############################################################################################################################################
##########################To calculate early penta3 and adjust timely penta3 and delayed penta3 and write adjusted outputs

inv.linpred_timely <- inv.linpred

load(file=paste0("simprob_penta3_delayed.rda"))
...
writeRaster(rr.mean, paste0("inla_vax_penta3_", ind.current, "_mean_adj.tif"), overwrite=TRUE)
writeRaster(rr.sd,   paste0("inla_vax_penta3_", ind.current, "_sd_adj.tif"), overwrite=TRUE)
writeRaster(rr.low,  paste0("inla_vax_penta3_", ind.current, "_low_adj.tif"), overwrite=TRUE)
writeRaster(rr.up,   paste0("inla_vax_penta3_", ind.current, "_up_adj.tif"), overwrite=TRUE)
writeRaster(rr.med,  paste0("inla_vax_penta3_", ind.current, "_median_adj.tif"), overwrite=TRUE)
...
write.csv(dist_out, paste0("adm3_estimates_penta3", ind.current, "_adj.csv"))
...
#The district-level estimates will have the same ordering as in the shapefile if they have the same no of areas
write.csv(dist_out, paste0("adm2_estimates_penta3_", ind.current, "_adj.csv"))
...
#The district-level estimates will have the same ordering as in the shapefile if they have the same no of areas
write.csv(dist_out, paste0("adm1_estimates_penta3_", ind.current, "_adj.csv"))
...
writeRaster(rr.50, paste0("inla_penta3_", ind.current, "_50perc_thresh_adj.tif"), overwrite=TRUE)
...
write.csv(data.frame(ID = 1:nrow(spol1), num.delayed.adm3, totchild = totchild.adm3), "est_num_delayed_penta3_adm3.csv")
write.csv(data.frame(ID = 1:nrow(spol2), num.delayed.adm2, totchild = totchild.adm2), "est_num_delayed_penta3_adm2.csv")
write.csv(data.frame(ID = 1:nrow(spol3), num.delayed.adm1, totchild = totchild.adm1), "est_num_delayed_penta3_adm1.csv")
...
#1. read in tiff image (inla_vax_penta3_delayed_mean)
penta3_delayed_pixel <- raster(paste0("inla_vax_penta3_delayed_mean_adj.tif"))

#2. actual plot
revMagma <- rasterTheme(region = rev(magma(10))) #this is needed to reverse the order of the levelplot default colour
levelplot(penta3_delayed_pixel, par.settings = revMagma, margin=FALSE, xlab = "", ylab = "") #margin=FALSE removes margin plot

tg <- textGrob("Modelled delayed PENTA3 at 1km 1 km pixel", x = unit(0.08, "npc"), y = unit(0.87, "npc"), hjust = 0, gp = gpar(fontsize = 14))
grid.draw(tg)

#3. save the image as PNG using name (pixel_delayed_penta3) with aspect ratio with = 1000 and height = 400
...
#1. read in tiff image (inla_vax_penta3_timely_mean)
penta3_timelypixel <- raster(paste0("inla_vax_penta3_timely_mean_adj.tif"))
...
levelplot(penta3_timelypixel, par.settings = revViridis, main = "PENTA3 timely", margin=FALSE) #margin=FALSE removes margin plot

#3. save the image as PNG using name (pixel_timely_penta3) with aspect ratio with = 1000 and height = 400

#########################################################################################

#early

#1. read in tiff image (inla_vax_penta3_early_mean)
penta3_earlypixel <- raster(paste0("inla_vax_penta3_early_mean_adj.tif"))
...
levelplot(penta3_earlypixel, col.regions = clrtopo(16), margin=FALSE, xlab = "", ylab = "") #margin=FALSE removes margin plot

tg <- textGrob("Modelled early PENTA3 at 1km 1 km pixel", x = unit(0.08, "npc"), y = unit(0.87, "npc"), hjust = 0, gp = gpar(fontsize = 14))
grid.draw(tg)

#3. save the image as PNG using name (pixel_early_penta3) with aspect ratio with = 1000 and height = 400
