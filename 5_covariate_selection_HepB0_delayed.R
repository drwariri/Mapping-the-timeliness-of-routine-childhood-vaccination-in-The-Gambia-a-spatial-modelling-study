...
#sd(out_dat1$hepB0_timely_prop)
#sd(out_dat1$hepB0_delayed_prop)
...
zero.clust <- which(is.na(data.merge$total_hepB0)|data.merge$total_hepB0<=1) #NOTE THAT CLUSTERS WITH SS<=1 HAVE BEEN DELETED
if (length(zero.clust)>0){
  data.merge <- data.merge[-zero.clust,]
}

#Outcome variable
total_sampled <- data.merge$total_hepB0 # Total number of vaccinated individuals/children surveyed in each location
num_success   <- data.merge$hepB0_delayed_count # Number who had delayed vaccination
...
plotsingles1=ggplot(meltdat, aes(x=value, y=out_emp_logit, xlab="")) +
  ...
  labs(x="", y="Empirical logit (HepB0_delay)")+
...
#Save plot 
pdf("cov_plot_emp_logit.pdf", height = 7, width = 7, pointsize = 6)
plotsingles1
dev.off()
...
GMB_n12_dst_edge_cultivated_areas_2015                                  GMB_n42_elevation_model 
1.430270                                                 1.888747 
l_GMB_n66_pigs_density                     GMB_n94_avg_number_WetDays_2016_2019 
1.543244                                                 2.417550 
GMB_n95_proximity_national_borders GMB_n102_avg_potential_evapotranspiration_2016_2019_mean 
1.359816                                                 2.806451                      1.098186                        1.003286 


#Write selected covariate information for the next step of the analysis
covout <- c("GMB_n12_dst_edge_cultivated_areas_2015",
            "GMB_n42_elevation_model",
            "l_GMB_n66_pigs_density",
...
write.csv(Data.out1, "HepB0_covariates_selected.csv")
