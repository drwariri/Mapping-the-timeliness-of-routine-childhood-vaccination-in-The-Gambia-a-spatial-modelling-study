...
dat.cov  <- read.csv("mcv1_delayed_covariates_selected.csv", header=TRUE)
...
# Observation points
X0 <- model.matrix(~ -1 + GMB_n94_avg_number_WetDays_2016_2019 + 
...
#Data
data <- data.frame(mcv1_delayed_count = data.merge$mcv1_delayed_count, total_mcv1 = data.merge$total_mcv1, Xobs,
...
form.inla = mcv1_delayed_count ~ 1 + GMB_n94_avg_number_WetDays_2016_2019 + 
  GMB_n99_avg_modis_Night_time_landsurface_temp_2016.2019_mean + 
  GMB_n102_avg_potential_evapotranspiration_2016_2019_mean + factor.urban_rural.1 + f(DHSCLUST, model = "iid", hyper = hyper)

inla.mod <- inla(form.inla, data = data.frame(data), family = "binomial", Ntrials = total_mcv1,
...
plot(vgm1) #Empirical variogram, without line
...
plot(vgm1$dist, vgm1$gamma, ylim=c(0,0.0003), col = "cornflowerblue", xlab = "distance", ylab = "semivariance")
