...
#sd(out_dat1$mcv1_timely_prop)
#sd(out_dat1$mcv1_delayed_prop)
#sd(out_dat1$mcv1_early_prop)
...
zero.clust <- which(is.na(data.merge$total_mcv1)|data.merge$total_mcv1<=1) #NOTE THAT CLUSTERS WITH SS<=1 HAVE BEEN DELETED
if (length(zero.clust)>0){
  data.merge <- data.merge[-zero.clust,]
}

#Outcome variable
total_sampled <- data.merge$total_mcv1 # Total number of vaccinated individuals/children surveyed in each location
num_success   <- data.merge$mcv1_delayed_count # Number who had delayed vaccination
...
plotsingles1=ggplot(meltdat, aes(x=value, y=out_emp_logit, xlab="")) +
  ...
  labs(x="", y="Empirical logit (mcv1_delay)")+
...
#Save plot 
pdf("cov_plot_emp_logit_mcv1_delay.pdf", height = 7, width = 7, pointsize = 6)
plotsingles1
dev.off()
...
GMB_n94_avg_number_WetDays_2016_2019 GMB_n99_avg_modis_Night_time_landsurface_temp_2016.2019_mean 
2.847201                                                     2.944535 
...
covout <- c("GMB_n94_avg_number_WetDays_2016_2019",
...
write.csv(Data.out1, "mcv1_delayed_covariates_selected.csv")