...
dat.cov  <- read.csv("HepB0_covariates_selected.csv", header=TRUE)
...
#Covariate names
covnames <- names(data.merge)[9:14]

# Observation points
X0 <- model.matrix(~ -1 + GMB_n12_dst_edge_cultivated_areas_2015 + 
                                GMB_n42_elevation_model + l_GMB_n66_pigs_density + 
...
#Data
data <- data.frame(hepB0_delayed_count = data.merge$hepB0_delayed_count, total_hepB0 = data.merge$total_hepB0, Xobs,
...
form.inla = hepB0_delayed_count ~ 1 + GMB_n12_dst_edge_cultivated_areas_2015 + 
  GMB_n42_elevation_model + l_GMB_n66_pigs_density + 
  GMB_n94_avg_number_WetDays_2016_2019 + 
  GMB_n95_proximity_national_borders + 
  GMB_n102_avg_potential_evapotranspiration_2016_2019_mean + factor.urban_rural.1 + f(DHSCLUST, model = "iid", hyper = hyper)

inla.mod <- inla(form.inla, data = data.frame(data), family = "binomial", Ntrials = total_hepB0,
...
plot(vgm1$dist, vgm1$gamma, ylim=c(0,0.12), col = "cornflowerblue", xlab = "distance", ylab = "semivariance")
